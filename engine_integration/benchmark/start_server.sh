...
if [ "$op" == "vllm" ]; then
    ...
    vllm serve "$MODEL" --disable-log-requests --no-enable-prefix-caching --enforce-eager --port="$VLLM_PORT"
elif [ "$op" == "sgl" -o "$op" == "sglang" ]; then
    source "$ENGINE_DIR/sglang-v0.4.6.post2/.venv/bin/activate"
    export PYTHONPATH="$KVCACHED_DIR:$PYTHONPATH"
    export TORCHINDUCTOR_DISABLE=1
    export TORCHDYNAMO_DISABLE=1
    python -m sglang.launch_server --model "$MODEL" --disable-radix-cache --trust-remote-code --attention-backend torch_native --port "$SGL_PORT"
else
